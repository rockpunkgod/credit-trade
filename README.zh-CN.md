...
用量记录包含来源、最终状态、执行结果、模式版本和内容摘要，但不保存提示词或输出。计价策略会快照价格与费率版本，并支持“每一百万 token 若干最小货币单位”等精确有理费率。所有数量和金额继续使用十进制字符串与 `bigint`，不使用浮点数记账；舍入规则会明确记录，当前按每条用量记录执行。
...
已经实现的模块边界和仍待解决的生产缺口见 [`docs/adr/0003-sandbox-billing-metering-boundary.md`](docs/adr/0003-sandbox-billing-metering-boundary.md)。
...
- `packages/core/src/billing`：纯计量模式、用量记录与精确计价函数
